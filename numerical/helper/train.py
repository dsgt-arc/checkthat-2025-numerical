...
from numerical.helper.util import format_time
...
def train(
    ...
    **kwargs,
    ...
    for epoch in range(epochs):
        ...
        classification_report = compute_class_report(val_labels_decoded, val_preds_decoded)
            ...
            log_fn(
                {
                    ...
                    "val/false_class_f1": classification_report.filter(pl.col("category") == "False")["f1-score"][0],
                    "val/conflicting_class_f1": classification_report.filter(pl.col("category") == "Half True/False")["f1-score"][
                        0
                    ],
                    "val/true_class_f1": classification_report.filter(pl.col("category") == "True")["f1-score"][0],
                }
            )
    ...
    logging.info(f"Training complete. Duration: {elapsed_training_time}. Best macro-F1: {best_macro_f1:.4f}")

    return model, best_macro_f1, best_val_preds


def compute_token_stats(
    train_input_ids: torch.Tensor, val_input_ids: torch.Tensor, test_input_ids: torch.Tensor, test_wo_labels: bool = False
) -> None:
