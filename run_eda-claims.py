...
from numerical.models.encoder import EncoderModel
from numerical.helper.data_store import DataStore
from numerical.models.dim_red import DimRed
from numerical.helper.visualization import Visualization
from numerical.helper.run_config import RunConfig

def init_args_parser() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Embedding Clustering Visualization")
...
def embed(model_settings: dict[str, str | int | bool ], df: pl.DataFrame) -> np.array:
    ...
    encoded_sequence = m.encode(df["claim"].to_list(), encode_settings=model_settings)
...
def main() -> int:
    ...
    try:
        ...
        ds.read_data(rc.data['train_claims_quantemp'])
        df = ds.get_data()
        ...
        train_claims_stats = (
            ...
            .describe()
            .rename({"value": "docs"})
            .join(df["claim"].str.len_chars().describe().rename({"value": "claim"}), on="statistic")
        )
        logging.info(f"Stats of train_claiims: \n{train_claims_stats}")
        ...
        embedding_path = Path(ds.location) / Path(rc.data['train_claims_embedding'] + '_' + encoder_model_name.replace("/", "-") + '.npy') 
